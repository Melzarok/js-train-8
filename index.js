...
function removeLastElement(arr) {
  ...
  return arr.length === 0 ? undefined : (arr.pop(), arr);
}
...
const filterByCondition = (arr, condition) => arr.filter(condition); //Коротший варіант
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    ...
    if (firstIndex !== -1) {
      ArrayOfIndex.push(firstIndex);
    }
    if (firstIndex !== lastIndex) {
      ArrayOfIndex.push(lastIndex);
    }
...
// console.log(findElementIndexes([1, 2, 3, 4, 5, 2], 10)); //Виведе [ 1, 5 ]
...
function customAt(arr, index) {
  ...
  if (!Array.isArray(arr) || index > arr.length || index < 0) {
    ...
    if (typeof element === "string") {
      ...
      return "Елемент є обєктом" + element;
    } else if (typeof element === "number") {
      return "Елемент є числом." + element;
    }
  }
}
